...
def get_video_topics(path):
    with AnyReader([path], default_typestore=typestore) as reader:
        connections = [x.topic for x in reader.connections if x.msgtype == 'sensor_msgs/msg/Image']
...
def create_video(data, topic):
    ...
    print(filename)
    video = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc(*"mp4v"), 30, (width, height), isColor=channels == 3)
