...
class TagTestCase(TestCase):
    ...
    def test_unique_violation(self):
        ...
        self.assertRaises(
            IntegrityError,
            Tag.objects.create, id=id, name=name)
...
class RestAPITagTestCase(APITestCase):
    ...
    def test_get_tags(self):
        ...
        self.assertEqual(
            response.data,
            [
                {'id': Tag.objects.get(name=self.test_names[0]).id, 'name': self.test_names[0]},
                {'id': Tag.objects.get(name=self.test_names[1]).id, 'name': self.test_names[1]},
                {'id': Tag.objects.get(name=self.test_names[2]).id, 'name': self.test_names[2]},
                {'id': Tag.objects.get(name=self.test_names[3]).id, 'name': self.test_names[3]},
                {'id': Tag.objects.get(name=self.test_names[4]).id, 'name': self.test_names[4]}            
            ]
        )
    ...
    def test_create_tag(self):
        ...
        self.assertEqual(response.data,
                         { 'id': Tag.objects.get(name="test_create_tag").id, 'name': "test_create_tag"})
...
class RestAPIMissionTagsTestCase(APITestCase):
    def setUp(self):
        ...
        self.mission_tag = Mission_tags.objects.create(mission=self.mission, tag=self.tags[0])
    ...
    def test_delete_mission_tag(self):
        ...
        response = self.client.delete(reverse('delete_mission_tag', kwargs={'mission_id': self.mission.id, 'tag_name': self.tags[0].name}))
        
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        # Check if the mission-tag association is removed
        self.assertFalse(Mission_tags.objects.filter(mission=self.mission, tag=self.tags[0]).exists())

    def test_get_missions_by_tag(self):
        # Check missions related to a specific tag
        response = self.client.get(reverse('get_missions_by_tag', kwargs={'name': 'TestTag0'}))
    ...
    def test_get_tags_by_mission_id(self):
        ...
        response = self.client.get(reverse('get_tags_by_mission_id', kwargs={'id': self.mission.id}))
